...
func ListenToEvents() {
	...
	// @dev Replace with the actual ABI of the contract
	// @todo if we need to read events instead of subscribe
	...
	sub, err := client.SubscribeFilterLogs(context.Background(), query, eventChannel)
	if err != nil {
		log.Fatal(err)
	}
